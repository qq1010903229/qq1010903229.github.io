...
// 后端接口
live2d_settings['modelAPI']             = 'https://cdn.jsdelivr.net/gh/qq1010903229/bandori-l2d-api/';   // 自建 API 修改这里
...
//看板娘样式设置
live2d_settings['waifuSize']            = '300x375';    // 看板娘大小，例如 '280x250', '600x535'
...
function initModel(waifuPath, type) {
    ...
    $('.waifu-tool .fui-info-circle').click(function (){
        if(window.confirm("点“确定”进入看板娘的萌娘百科页面，“取消”进入如何添加看板娘页面"))window.open(waifu_tips.waifu.about[live2d_settings.modelStorage ? localStorage.getItem('modelId') : sessionStorage.getItem('modelId')]);
		else window.open(live2d_settings.aboutPageUrl);
    });
...
function loadModel(modelId, modelTexturesId=0) {
    ...
    } loadlive2d('live2d', live2d_settings.modelAPI+'get/'+modelId+'-'+modelTexturesId+'.json', (live2d_settings.showF12Status ? console.log('[Status]','live2d','模型',modelId+'-'+modelTexturesId,'加载完成'):null));
}
...
function loadTipsMessage(result) {
    ...
    function loadOtherModel() {
        ...
        $.ajax({
            cache: true,
            url: live2d_settings.modelAPI+modelRandMode+'/'+modelId+'.txt',
            dataType: "json",
            success: function(result) {
				...
				window.playmodelsound(result.model['id'],0);
				message="("+result.model['id']+"-0) "+message;
    ...
    function loadRandTextures() {
        ...
        $.ajax({
            cache: true,
            url: live2d_settings.modelAPI+modelTexturesRandMode+'_textures/'+modelId+'-'+modelTexturesId+'.txt',
            dataType: "json",
            success: function(result) {
                showMessage("("+modelId+"-"+result.textures['id']+") "+waifu_tips.load_rand_textures[modelId], 3000, true);
				window.playmodelsound(modelId,1);
	...
	function selfIntroduction(){
	  var modelId = modelStorageGetItem('modelId');
		$.each(waifu_tips.model_message, function(i,val) {if (i==modelId) message = getRandText(val)});
		window.playmodelsound(modelId,0);
		message="("+modelId+"-"+modelStorageGetItem('modelTexturesId')+") "+message;
        showMessage(message, 3000, true);
	}
    $('.waifu-tool .fui-eye').click(function (){loadOtherModel()});
    $('.waifu-tool .fui-user').click(function (){loadRandTextures()});
    $('.waifu-tool .fui-chat').click(function (){selfIntroduction()});
}
